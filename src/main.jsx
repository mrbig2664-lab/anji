...
const days = [
  ...
  { id: 'day2', label: 'Day 2', short: '漂流日', date: '7/29', ticket: 'WATER DAY', mood: '睡到自然醒，再把自己交给水流', title: '自然醒，然后去漂流', next: '确认漂流地点和出发时间', summary: '睡到自然醒，再把自己交给水流。夏天的快乐有时候不用高级，够凉就行。', checklist: ['手机防水袋', '干衣服', '毛巾', '防滑拖鞋', '防晒', '防蚊', '漂流后洗澡换衣'], task: '漂流后给对方拍一张狼狈但快乐的照片。', pending: ['漂流地点', '漂流门票', '晚饭地点'], plans: [['Morning', '自然醒，不设闹钟', '酒店早餐，慢慢收拾，不用把上午排满。'], ['Noon / afternoon', '漂流候选', '将军关 / 仙龙峡 / 十八道湾，先看距离、营业时间和体力。'], ['Evening', '回酒店洗澡休息', '吃一顿舒服的晚饭，今天不再安排新景点。']] },
  { id: 'day3', label: 'Day 3', short: '瀑布咖啡', date: '7/30', ticket: 'WATERFALL HOURS', mood: '有水声的下午，适合把话说慢一点', title: '瀑布咖啡与山里走走', next: '去瀑布咖啡', summary: '这一天不用赶很多地方。找一处有水声的咖啡，把下午交给山里。', checklist: ['相机 / 手机电量', '防晒', '防蚊', '舒服的鞋', '纸巾', '小外套'], task: '各自拍一张不用露脸、但很像对方的照片。', pending: ['瀑布咖啡具体店名', '是否附近徒步', '晚饭选择'], plans: [['Morning', '自然醒 / 酒店早餐', '看天气和体力，不急着出门。'], ['Noon / afternoon', '安吉瀑布咖啡', '具体店名待确认，先记住：喝咖啡、拍照、听水声。'], ['Late afternoon', '附近徒步 / 山里溜达', '走到刚刚好，傍晚再决定去哪里吃饭。']] },
...
function TodayMode({ stage, autoId, selectedId, setSelectedId, checklist, toggleChecklist, ready, toggleReady }) {
  ...
  return <section className="today-section content-section" id="today"><SectionHeading eyebrow="01 / Today mode" title="今天要做什么" copy="自动按北京时间显示，也可以提前看明天。" action={<span className="time-badge">Asia / Shanghai</span>} /><StageSwitcher selectedId={selectedId} autoId={autoId} setSelectedId={setSelectedId} /><div className="today-card reveal"><div className="today-head"><div><span className="today-date">{stage.date} · {stage.ticket}</span><h3>{stage.title}</h3><p>{stage.mood}</p></div><span className="auto-mark">{selectedId === autoId ? 'TODAY' : 'PREVIEW'}</span></div><div className="next-action"><span>下一件事</span><strong>{stage.next}</strong></div><div className="today-columns"><div><span className="field-label">今日要带 / 要做</span><div className="today-checks">{stage.checklist.slice(0, 7).map(item => { const key = `${stage.id}-${item}`; return <button className={checklist[key] ? 'checked' : ''} key={item} onClick={() => toggleChecklist(key)}><i>{checklist[key] ? '✓' : ''}</i>{item}</button> })}</div></div><div><span className="field-label">今日待确认</span><ul className="pending-list">{stage.pending.map(item => <li key={item}>{item}</li>)}</ul></div></div><div className="today-task"><span>今日照片任务</span><p>{stage.task}</p></div><div className="ready-row"><span className="field-label">出门前，两个人都点一下</span><div><PersonChoice person="jenny" checked={ready[selectedId]?.jenny} onClick={() => toggleReady(selectedId, 'jenny')} /><PersonChoice person="richard" checked={ready[selectedId]?.richard} onClick={() => toggleReady(selectedId, 'richard')} /></div>{bothReady && <strong className="all-set">All set. 出发。</strong>}</div></div></section>
}

function MemoryMode({ autoId, selectedId, setSelectedId, photos, memories, decisions, completedTasks, seal, setSeal }) {
  const tripCover = photos.find(photo => photo.isTripCover)
  const matches = decisionGroups.flatMap(group => group.items.map(item => ({ label: item[1], state: decisions[item[0]] })).filter(item => item.state?.jenny && item.state?.richard))
  const doneTasks = Object.entries(completedTasks).filter(([, value]) => value).map(([key]) => dailyTasks[Number(key)])
  const travelDays = days.filter(day => day.id.startsWith('day'))
  return <section className="memory-mode-section content-section" id="today"><SectionHeading eyebrow="01 / Memory mode" title="我们真的去过山里" copy="照片负责证明来过，句子负责证明当时我们在一起。" action={<span className="time-badge">after 08/01</span>} /><StageSwitcher selectedId={selectedId} autoId={autoId} setSelectedId={setSelectedId} /><div className="trip-cover reveal">{tripCover ? <img src={tripCover.src} alt={tripCover.caption || '旅行封面'} /> : <div className="cover-placeholder"><span>TRIP COVER</span><b>等一张这趟旅行的封面照</b><small>在 Photo Vault 里设为整趟封面</small></div>}<div className="cover-copy"><span>ANJI / MEMORY BOOK</span><h3>山里有风，也有我们</h3><p>Into the Green, With You</p></div></div><div className="memory-highlights"><div><b>{photos.length}</b><span>photos kept</span></div><div><b>{matches.length}</b><span>matches</span></div><div><b>{doneTasks.length}</b><span>tiny tasks</span></div></div><div className="match-strip"><span className="field-label">Match 过的选择</span>{matches.length ? matches.map(item => <span className="match-stamp" key={item.label}>● {item.label}</span>) : <p>还没有 Match，先从 Decision Board 里一起点一张。</p>}</div><div className="done-task-strip"><span className="field-label">完成过的小任务</span>{doneTasks.length ? doneTasks.map(task => <span key={task}>✓ {task}</span>) : <p>旅行中完成的小任务会出现在这里。</p>}</div><div className="memory-glance"><div className="memory-glance-heading"><span className="field-label">Four day memory cards</span><a href="#memory">继续写 Memory Log →</a></div><div className="memory-glance-grid">{travelDays.map(day => { const dayPhotos = photos.filter(photo => photo.day === day.id).slice(0, 6); const note = memories[day.id] || {}; return <article className="memory-glance-card" key={day.id}><div className="glance-photos">{dayPhotos.length ? dayPhotos.map(photo => <img key={photo.id} src={photo.src} alt={photo.caption || `${day.label} photo`} />) : <div className="glance-placeholder">等一张{day.short}的照片</div>}</div><div className="glance-copy"><span>{day.label} · {day.date}</span><strong>{day.title}</strong><p>{note.jenny || note.richard || '这一页还空着，等你们各留一句。'}</p></div></article> })}</div></div><SealThisTrip seal={seal} setSeal={setSeal} /></section>
}

function PlanSection({ activeDay, setActiveDay, completed, togglePlan }) {
  const day = days.find(item => item.id === activeDay) || days[1]
  const travelDays = days.filter(item => item.id.startsWith('day'))
...
function ChecklistSection({ checklist, toggleChecklist, activePack, setActivePack }) { const group = packGroups.find(item => item.id === activePack); return <section className="packing-section content-section" id="packing"><SectionHeading eyebrow="04 / Practical checklist" title="出门前，少想一点" copy="带没带，点一下就知道。" /><div className="pack-tabs">{packGroups.map(item => <button className={activePack === item.id ? 'active' : ''} key={item.id} onClick={() => setActivePack(item.id)}>{item.label}<small>{item.items.length}</small></button>)}</div><div className="pack-card"><div className="pack-card-head"><span>{group.label}</span><b>{group.items.filter(item => checklist[`pack-${activePack}-${item}`]).length} / {group.items.length}</b></div><div className="pack-grid">{group.items.map(item => { const key = `pack-${activePack}-${item}`; return <button className={checklist[key] ? 'checked' : ''} key={item} onClick={() => toggleChecklist(key)}><i>{checklist[key] ? '✓' : ''}</i>{item}</button> })}</div></div></section> }
...
function QuickActions() { return <section className="quick-section content-section" id="quick"><SectionHeading eyebrow="08 / Quick actions" title="要用的时候，在这里" copy="真实链接确认后再补，先把位置留好。" /><div className="quick-grid"><a href="#top" className="quick-card"><span>⌂</span><b>酒店地址</b><small>安吉中南度假 · 东非草原酒店</small></a><a href="#top" className="quick-card"><span>↗</span><b>导航到酒店</b><small>链接待补</small></a><a href="#decisions" className="quick-card"><span>⌁</span><b>漂流地点</b><small>候选待确认</small></a><a href="#decisions" className="quick-card"><span>✦</span><b>瀑布咖啡</b><small>具体店名待确认</small></a><a href="#today" className="quick-card"><span>☼</span><b>天气查看</b><small>出发前再看一次</small></a><a href="#memory" className="quick-card"><span>＋</span><b>备注区</b><small>写进 Memory Log</small></a></div></section> }
